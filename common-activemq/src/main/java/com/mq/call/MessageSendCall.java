...
public class MessageSendCall {
	...
	public synchronized static void topicProxyTextMessageSend(MsgCommand cmd) throws MsgException {
		...
		// 进行对象绑定
		IMessageTopicSender messageTopicSend = (IMessageTopicSender) messageInvocationHandler.bind(new MessageTopicSender());
		try {
			// 命令对象不能空 ，发送消息对象不能空
	...
	public static void topicProxyOjbectMessageSend(MsgCommand cmd) throws MsgException {
		// 线程锁当前方法参数对象
		synchronized (cmd) {
			// 实例化代理对象
			MessageInvocationHandler messageInvocationHandler = new MessageInvocationHandler(new MessageTopicSender());
	...
	public synchronized static void queueProxyTextMessageSend(MsgCommand cmd) throws MsgException {
		...
		// 进行对象绑定
		IMessageQueueSender messageQueueSend = (IMessageQueueSender) messageInvocationHandler.bind(new MessageQueueSender());
