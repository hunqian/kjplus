...
public class ImageIOUtil {
	 ...
	 * @return
	 */
	public static boolean resizeImage(String imgSrcPath, String imgDestPath,
			int maxEdgeLength) throws Exception {
		...
		String fileType = "jpg";
		int pos = imgSrcPath.lastIndexOf(".");
		if (pos > -1)
			fileType = imgSrcPath.substring(pos + 1);
		if (fileType.length() > 3)
			fileType = fileType.substring(0, 3);
		/*
		 * if(imgSrcPath.contains(".jpg")) fileType = "jpg"; else
		 * if(imgSrcPath.contains(".png")) fileType = "png";
		 */
		...
		try {
			...
			tag.getGraphics().drawImage(
					...
					0, 0, null);
			out = new FileOutputStream(imgDestPath);
			ImageIO.write(tag, fileType, out);
		} catch (Exception e) {
			throw new RuntimeException("resize image error, img=" + imgSrcPath,
					e);
		} finally {
			if (out != null) {
				out.close();
	...
	public static int compressImage(String imageFullPath, int quality,
			long maxFileSize) {
		// 1. entry validation
		if (Util.isNull(imageFullPath) || quality < 0 || quality > 100) {
	...
	public static ImageFile getImageInfo(String localImagePath, boolean isReadExif) {
		File imgFile = new File(localImagePath);
		if (!imgFile.isFile()) {
	...
	/*
	 * private static ImageExif readImageExif(MagickImage image) { ImageExif
	 * exif = new ImageExif(); if (image == null) { return exif; } try { for
	 * (Method method : exifMethods) { if
	 * ("set".equals(method.getName().substring(0, 3))) { if
	 * (method.getName().length() > 3) { method.invoke(exif,
	 * image.getImageAttribute("EXIF:" + method.getName().substring(3))); } } }
	 * 
	 * } catch (Exception e) { throw new
	 * RuntimeException("read image exif error", e); } return exif; }
	 */
	...
	public static BufferedImage loadImage(String url,String file,String format){
		try{
	...
	public static void main(String[] args) {
		int maxEdgeLength = 100;
		String path = "D:/Post/image/";
		String file = "201312814193986670.png";
		String destFile = file.substring(0, file.length() - 4) + "_"
				+ maxEdgeLength + file.substring(file.length() - 4);
		// String src = "D:/Post/image/2012121510431561737.jpg";
		// String desc = "D:/Post/image/2012121510431561737.jpg";
		/*try {
			ImageIOUtil.resizeImage(path + file, path + destFile, maxEdgeLength);
		} catch (Exception e) {
		}*/
		String url = "http://fb.fbcontent.cn/api/xingce/accessories/formulas?latex=EQUFMx4D4B34CSLlI%2FQVf2JxCqL%2FcgvFFMdLR%2FEk0vIYzFKInYH2PcuNz9%2BPBxvxyGDt3cW7Vv0Eltv%2FV0%2F5vQ%3D%3D";
		String fl = "e:/temp/123456.png";
		ImageIOUtil.loadImage(url, fl, "PNG");
	}
}