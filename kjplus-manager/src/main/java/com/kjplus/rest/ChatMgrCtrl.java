...
@Controller
public class ChatMgrCtrl {
	// 日志
	private Logger logger = Logger.getLogger(ChatMgrCtrl.class);
	...
	public ModelAndView chatContucts(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			SysFuncEbo func = funcService.getFuncByMenu("contactslist.html", Constant.MENU_TYPE_MANAGER);
			if (func != null)
			...
			if (userEbo.getSessionCode() == null || userEbo.getSessionCode() == "") {
			...
			List<ChatContactsDto> rContacts = chatMsgService.recentContacts(thisSessionCode);
			map.put("rContacts", rContacts);
			if (rContacts.size() > 0) {
				map.put("firstcontact", rContacts.get(0));
			}

			List<ChatContactsDto> contacts = chatMsgService.chatContactsList(thisSessionCode, null, 0, 10);
	...
	public @ResponseBody
	Map<String, Object> createDocument(HttpServletRequest request, HttpServletResponse response) {
		...
		synchronized (request) {
			...
			//String restMessage = message.substring(message.indexOf("<img>") + 5, message.indexOf("</img>"));
			try {
				KjAdminUserDto userEbo = (KjAdminUserDto) request.getSession().getAttribute(Constant.KJMGR_SESS);
				...
				chatMsgService.updateLatelyChatTime(userEbo.getSessionCode(), sessionCode);
				map.put("msg", msg);
				map.put("result", 1);
	...
	// 获取最近聊天记录
	@RequestMapping(value = "/mgr_getchatslistjson.html")
	public @ResponseBody
	Map<String, Object> getChatListJson(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			List<ChatMsgDto> listChats = chatMsgService.listChats(userEbo.getSessionCode(), toSessionCode);
			if (listChats == null) {
				map.put("data", "没有相关消息");
				map.put("result", -1);
				return map;
	...
	// 最近联系人列表重排
	@RequestMapping(value = "/mgr_getlatelycontacts.html")
	public @ResponseBody
	Map<String, Object> getLatelyContacts(HttpServletRequest request, HttpServletResponse response) {
		...
		KjAdminUserDto userEbo = (KjAdminUserDto) request.getSession().getAttribute(Constant.KJMGR_SESS);
		try {
			List<ChatContactsDto> recentContacts = chatMsgService.recentContacts(userEbo.getSessionCode());
