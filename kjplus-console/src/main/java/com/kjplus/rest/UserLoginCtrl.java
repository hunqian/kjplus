...
public class UserLoginCtrl {
	...
	public ModelAndView login(HttpServletRequest request, HttpServletResponse response) {
		...
		KjAdminUserDto user = (KjAdminUserDto) request.getSession().getAttribute(Constant.KJCONSOLE_SESS);
		try {
			if (user != null) {
	...
	public ModelAndView logout(HttpServletRequest request, HttpServletResponse response) {
		request.getSession().removeAttribute(Constant.KJCONSOLE_SESS);
	...
	public @ResponseBody Map<String, Object> loginj(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String vericode0 = (String) request.getSession()
					.getAttribute(Constant.KJ_CONSOLE_SESS_VERICODE + "login");
			...
			request.getSession().setAttribute(Constant.KJCONSOLE_SESS, adminUser);
		} catch (DataException e) {
			logger.error(e);
	...
	public @ResponseBody void veriCode(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if ("mblogin".equals(veritype))
				...
				code = Util.genDigiCodeStr(6);
			request.getSession().setAttribute(Constant.KJ_CONSOLE_SESS_VERICODE + veritype, code);
