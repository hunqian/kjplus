...
@Controller
public class SignListMgrCtrl {

	private static Logger logger = Logger.getLogger(SignListMgrCtrl.class);
	@Autowired
	...
	public ModelAndView createDocument(HttpServletRequest request, HttpServletResponse response) {
		...
		if (userEbo == null) {
			...
			map.put("result", -1);
			map.put("returnUrl", "/signlist.html");
			map.put("message", "用户没有登陆!");
			return mav;
		}
		try {
			SysFuncEbo func = funcService.getFuncByMenu("signlist.html", Constant.MENU_TYPE_MANAGER);
			if (func != null)
