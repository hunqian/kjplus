...
Msgtmp = {
	...
	initTable:function(){
		...
		oMsgtmpTable = $('#msgtmplist-table').DataTable({
			...
			"sAjaxSource": base + "/mgr_wxmsgtmplistjson.html",
			"columnDefs" :[
							{
							    "targets": 8,
							    "data": null,
							    "render": function(data, type, row) {
							    	var html = "";
									html += '<button class="btn btn-xs btn-danger" id="getmsgcontent'+ data[1] + '"><i class="icon-cogs bigger-120">内容</i></button>';
									return html;
							    }
							}
						],
			...
			"fnInitComplete": function() {
				...
				$("#btnSearch").click(function(){
					var u = base + '/mgr_wxmsgtmplistjson.html?a=a';
				...
				$("#btnempty").click(function(){
					$('#qh_title').val("");
					$('#qh_code').val("");
					var u = base + '/mgr_wxmsgtmplistjson.html';
					oMsgtmpTable.ajax.url(u).load();
				});
		...
		oMsgtmpUseTable = $('#msgtmp_userlist-table').DataTable({
			...
			"columnDefs" :[{ 
							...
							{/*
							    ...
							    "render": function(data, type, row) {
							    	var html = "";
									html += '<button class="btn btn-xs btn-danger" id="editmagtmp'+ data[0] + '"><i class="icon-cogs bigger-120">标题</i></button>';
									html += '<button class="btn btn-xs btn-danger" id="getmsgcontent'+ data[1] + '"><i class="icon-cogs bigger-120">内容</i></button>';
									return html;
							    }
							*/}
			...
			"aoColumns": [
			          	...
			          	{ "sData": "标题", "bSortable": false, "bVisible": true},
			          	{ "sData": "内容", "bSortable": false, "bVisible": true,"width":"20%"},
	...
	rebindEvent:function(){
		...
		$("[id^='editmsgtmp']").unbind().click(function(){
			...
			console.log(id);
			$.ajax({
				url: base + '/mgr_wxmsgtmpltjson.html?libid=' + id,
				type: 'GET',
				dataType: 'json',
				...
				buttons: [
							{
								...
								click: function() {
									...
									console.log(reqData);
									$.ajax({
										url: base + '/mgr_updateMsgTmpljson.html',
										data: reqData,
										type: 'POST',
										dataType: 'json',
										success: function (resp) {
