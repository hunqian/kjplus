...
@Controller
public class AuditedListMgrCtrl {

	private static Logger logger = Logger.getLogger(AuditedListMgrCtrl.class);
	@Autowired
	...
	public ModelAndView createDocument(HttpServletRequest request, HttpServletResponse response) {
		...
		if (userEbo == null) {
			...
			map.put("result", -1);
			map.put("returnUrl", "/audited.html");
			map.put("message", "用户没有登陆!");
			return mav;
		}
		try {
			SysFuncEbo func = funcService.getFuncByMenu("audited.html", Constant.MENU_TYPE_CONSOLE);
			if (func != null)
