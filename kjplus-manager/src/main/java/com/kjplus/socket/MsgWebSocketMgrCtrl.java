...
public class MsgWebSocketMgrCtrl {

	private static Logger logger = Logger.getLogger(MsgWebSocketMgrCtrl.class);

	// key 作为用户的标识 session作为用户 所对应的会话
	private static ConcurrentMap<String, Object> sessionInfoMap = new ConcurrentHashMap<String, Object>();

	// 用户 信息 全部加入缓存已提高系统性能
	private static ConcurrentMap<String, Object> userInfoMap = new ConcurrentHashMap<String, Object>();

	// 线程此 统一操作消息发送事件 进行消息推送 免去线程创建销毁时的耗时操作
	// private ThreadPoolTaskExecutor poolTaskExecutor =
	// SpringUtil.getApplicationContext().getBean(ThreadPoolTaskExecutor.class);
	...
	private static ISessionSocketService sessionService = SpringUtil.getApplicationContext().getBean(ISessionSocketService.class);
	...
	public void onOpen(Session session, EndpointConfig config) throws IOException {
		synchronized (session) {
			...
			// 拿到登录用户的相关信息
			KjAdminUserDto user = (KjAdminUserDto) httpSession.getAttribute(Constant.KJMGR_SESS);
			// 判断用户是否登陆
			String jsonMessage = "";
			if (user != null) {
				...
				if (isNullSCode) {// 用户登录没有带sessionCode
					try {
						...
						SessionSocketEbo ss = sessionService.getSessionByUid(uid, Constant.FLAG_YES);
...
//							session.getBasicRemote().sendText(jsonMessage);
							...
							sessionService.addSessionSocket(ssk);
...
//				jsonMessage = JSONObject.toJSONString("websoket连接失败,请登录！");
//				session.getBasicRemote().sendText(jsonMessage);
			}
		}
	}
	...
	public static void sendMessage(MessageDto message) throws IOException {
		synchronized (message) {
			...
			try {
				SessionSocketEbo sessionByCode = sessionService.getSessionByCode(message.getFrSCode(), Constant.FLAG_YES);
				SessionSocketEbo sessionByCode2 = sessionService.getSessionByCode(message.getContSCode(), Constant.FLAG_YES);
