...
public class Util {
	...
	public static String getTimeDiff(Date time) {
		...
		// 不同一天
		if (c.get(Calendar.DAY_OF_YEAR) > t.get(Calendar.DAY_OF_YEAR) + 3)
			return dayFormat.format(time);
		else if (c.get(Calendar.DAY_OF_YEAR) >= t.get(Calendar.DAY_OF_YEAR) + 1)
			return (c.get(Calendar.DAY_OF_YEAR) - t.get(Calendar.DAY_OF_YEAR)) + "天前";
		else {
			if (c.get(Calendar.HOUR_OF_DAY) == t.get(Calendar.HOUR_OF_DAY)){
				if(c.get(Calendar.MINUTE) < t.get(Calendar.MINUTE))
					return "立刻";
				else
					return (c.get(Calendar.MINUTE) - t.get(Calendar.MINUTE)) + "分钟前";
			}else if (c.get(Calendar.HOUR_OF_DAY) == t.get(Calendar.HOUR_OF_DAY) + 1) {
				if (c.get(Calendar.MINUTE) >= t.get(Calendar.MINUTE)) {
					return (c.get(Calendar.HOUR_OF_DAY) - t.get(Calendar.HOUR_OF_DAY)) + "小时前";
				} else {
					// 下个一小时
					return (60 + c.get(Calendar.MINUTE) - t.get(Calendar.MINUTE)) + "分钟前";
				}
			} else
				return (c.get(Calendar.HOUR_OF_DAY) - t.get(Calendar.HOUR_OF_DAY)) + "小时前";
		}*/
	...
	// 通过正则表达式返回匹配内容
	public static String getRexStr(String str, String expression) {
		if (Util.isNull(str))
			return null;
		try {
			org.apache.oro.text.regex.PatternCompiler compiler = new org.apache.oro.text.regex.Perl5Compiler();
			org.apache.oro.text.regex.Pattern pattern = compiler.compile(expression);
	...
	//获得一个字符中包含的doble值,空值返回0，非法字符返回-1
	public static double getDoubleVl(String str){
		if(Util.isNull(str))
			return 0;
		String v = Util.getRexStr(str, "[0-9\\.]{1,}");
		if(Util.isNull(v))
			return 0;
		try{
	...
	//获得一个字符中包含的int值,空值返回0，非法字符返回-1
	public static int getIntVl(String str){
		if(Util.isNull(str))
			return 0;
		String v = Util.getRexStr(str, "[0-9]{1,}");
		if(Util.isNull(v))
	...
	private static void testUUID(int total){
		Hashtable<String,String> uuids = new Hashtable<String,String>();
		String uustr = null;
		for(int i=0;i<total;i++){
			UUID uuid = UUID.randomUUID();
			uustr = uuid.toString();
			if(uuids.containsKey(uustr))
	...
	public static int listSize(List list){
		...
		for(int i=0;i<list.size();i++){
			obj = list.get(i);
			if(obj == null || Util.isNull(obj.toString()))
				return i;
		}
		return list.size();
	}
	...
	//返回列表的字符串，用逗号间隔
	public static String arr2Str(List<Integer> entityIds) {
	...
	public static void main(String args[]){
        ...
        System.out.println(Util.genSessCode(32));
        System.out.println(Util.genSessCode(32));
        System.out.println(Util.genSessCode(32));
        System.out.println(Util.genSessCode(32));
        System.out.println(Util.genSessCode(32));
