    ...
    $.extend( calendar.prototype, {
        ...
        _initEvent: function() {
            ...
            this.el.delegate( 'ul.day li', 'click', function( event ) {
                var dateLis = document.querySelector('.day').querySelectorAll('li');
                for (var i = 0, l = dateLis.length; i < l; i++) {
                    dateLis[i].style.background = '';
                    dateLis[i].style.borderRadius = '0';
                    dateLis[i].style.color = '#333';
                }
                if (document.querySelector('.dd')) {
                    var today = document.querySelector('.dd');
                    today.style.background = '#03C7AD';
                    today.style.borderRadius = '1.375rem';
                    today.style.color = '#fff';
                }
                if (this.innerText != '') {
                    this.style.background = '#0bd16b';
                    this.style.borderRadius = '1.375rem';
                    this.style.color = '#fff';
                    console.log(this.getAttribute('data-date'));
                }

                //点击的日期
                // var liDate = this.getAttribute('data-date');
                //添加活动按钮
                // var addActivity = document.querySelector('.add_activity');
        ...
        //设置活动日期
        activity: function () {
            var dateLis = document.querySelector('#calendar').querySelector('.day').querySelectorAll('li');
            for (var i = 0; i < dateLis.length; i++) {
                if (dateLis[i].getAttribute('data-date') == '2017-08-18') {
                    console.log(dateLis[i].querySelectorAll('i')[1])
                    dateLis[i].querySelectorAll('i')[1].innerHTML = '活动';
                    dateLis[i].querySelectorAll('i')[1].style.color = '#fff';
                    dateLis[i].querySelectorAll('i')[1].style.backgroundColor = '#0BD16B';
                    dateLis[i].querySelectorAll('i')[1].style.display = 'block';
                    dateLis[i].querySelectorAll('i')[1].style.width = '2rem';
                    dateLis[i].querySelectorAll('i')[1].style.marginLeft = '0.375rem';
                    dateLis[i].querySelectorAll('i')[1].style.borderRadius = '0.25rem';
                    dateLis[i].className = 'hd';
                }
            }
        }
