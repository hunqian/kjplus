...
WxMenu = {
	...
	initTable:function(){
		oMenuListTable = $('#wxmenulist-table').DataTable({
			...
			"sAjaxSource": base + "/mgr_wxmenulistjson.html",
			"columnDefs" :[
			               { 
				            	 "targets": 5,
							    "data": null,
			               ...
			               { 
						...
						},
							{
							    ...
							    "render": function(data, type, row) {
							    	var html = "";
									html += '<button class="btn btn-xs btn-info" id="updatemenu'+ data[0] + '"><i class="icon-cogs bigger-120">修改</i></button>';
									if(data[3] == 0)
										html += '<button class="btn btn-xs btn-info" id="addmenu'+ data[0] + '"><i class="icon-cogs bigger-120">增加</i></button>';
									return html;
							    }
							}
						],
			...
			"aoColumns": [
						...
						{ "sName": "父Id", "bSortable": false, "bVisible": false},
				...
				"fnInitComplete": function() {
					...
					$("#btnSearch").click(function(){
						var u = base + '/mgr_wxmenulistjson.html?a=a';
						u += "&flag=" +$('#qh_flag').val();
						console.log(u);
			...
			buttons: [
						{
							...
							click: function() {
								...
								$.ajax({
									url: base + '/mgr_addmenujson.html',
									data: reqData,
									type: 'POST',
									dataType: 'json',
									success: function (resp) {
										var u = base + '/mgr_wxmenulistjson.html';
										//带组织
										oMenuListTable.ajax.url(u).load();
										alert(resp.message);
									}
					...
					buttons: [
								{
									...
									click: function() {
										...
										$.ajax({
											url: base + '/mgr_updatemenujson.html',
											data: reqData,
											type: 'POST',
											dataType: 'json',
											success: function (resp) {
