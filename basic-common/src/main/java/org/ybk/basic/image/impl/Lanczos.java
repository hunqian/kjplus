...
public class Lanczos {
	...
	public static BufferedImage resizeImage(BufferedImage image, int width, int height) {
		...
		int newHeight = 0;
		if(xFactor >= yFactor){
			newWidth = (int)(image.getWidth()*xFactor);
			newHeight = (int)(image.getHeight()*xFactor);
		}else{
			newWidth = (int)(image.getWidth()*yFactor);
			newHeight = (int)(image.getHeight()*yFactor);
		}
		resizeImage = new BufferedImage(newWidth,newHeight, image.getType());
		BufferedImage filterImage = new BufferedImage(newWidth, newHeight, image.getType());
		
		if(xFactor >= yFactor){
			verticalFilter(image, filterImage, xFactor);
			horizontalFilter(filterImage, resizeImage, xFactor);
		}else{
			horizontalFilter(image, filterImage, yFactor);
			verticalFilter(filterImage, resizeImage, yFactor);
		}*/
	}
