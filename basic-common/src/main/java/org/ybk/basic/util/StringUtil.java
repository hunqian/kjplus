...
public class StringUtil {
	...
	// 随机数字字符串
	public static String getRandDigiStr(int n) {
	...
	public static String getRandStr(int n) {
		...
		int schar = 0;
		for (int i = 0; i < n;) {
			schar = 97 + (int) (random.nextDouble() * 26);
			if (schar >= 97 && schar <= 123) {
				String rand = String.valueOf((char) schar);
				sRand += rand;
				i++;
			}
		}
	...
	public static boolean checkLegalChar(String vl){
		if(Util.isNull(vl))
			return true;
		//[^u]
		if(vl.matches("[a-zA-Z0-9_\u4e00-\u9fa5\\-]*")){
			return true;
		}else
			return false;
		/*if(vl.matches("[a-zA-Z0-9_\u4e00-\u9fa5\\w\uf900-\ufa2d\\-]*")){
            //不是非法字符
			return true;
        }else{
        	if(vl.matches("[—@\\.]*"))
