...
@Controller
public class FollowupStatisticsMgrCtrl {

	private static Logger logger = Logger.getLogger(FollowupStatisticsMgrCtrl.class);
	@Autowired
	...
	// 签约列表
	@RequestMapping("/followupstatistics.html")
	public ModelAndView createDocument(HttpServletRequest request, HttpServletResponse response) {
		...
		if (userEbo == null) {
			...
			map.put("result", -1);
			map.put("returnUrl", "/followupstatistics.html");
			map.put("message", "用户没有登陆!");
			return mav;
		}
		try {
			SysFuncEbo func = funcService.getFuncByMenu("followupstatistics.html", Constant.MENU_TYPE_MANAGER);
			if (func != null)
