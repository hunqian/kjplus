						...
						for (k in resp.data) {
							...
							for (var i = 0; i < resp.data[k].refVls.length; i++) {
								if (resp.data[k].lineMultiRef == "N"){
									html += '<div class="clearfix" data-id="'+ resp.data[k].lineCode+ '" data-req="'+ resp.data[k].isReq+ '" ata-title="'+ resp.data[k].lineTitle + '"  data-refvlid="'+ resp.data[k].refVls[i].refVlId+ '">' + resp.data[k].refVls[i].refVlName+ '</div>';
								}
