...
ServAss = {
	...
	initTable:function(){
		...
		oServAssTable = $('#servasslist-table').DataTable({
			...
			"sAjaxSource": base + "/mgr_listservassjson.html?status=Y",
			"columnDefs" :[
				{
				    "targets": 9,
				    "data": null,
			...
			"aoColumns": [
						...
						{ "sName": "服务包名", "bSortable": false, "bVisible": true},
						{ "sName": "简称", "bSortable": false, "bVisible": true},
			...
			"fnInitComplete": function() {
				...
				var condition = "";
				condition = "<label>医生/居民姓名:<input type=\"text\" placeholder=\"关键字\" id=\"qh_name\" /></label>"; 
				condition += "<label>签约状态:<select id=\"qh_status\" name=\"qh_status\" style=\"width:100px;\">";				condition += "<option value=\"\">全部</option>";
				...
				$("#btnSearch").click(function(){
					var u = base + '/mgr_listservassjson.html?a=a';
					u += "&name="+ $("#qh_name").val();
				...
				$("#btnempty").click(function(){
					$("#qh_name").val("");
					$("#qh_status").val("");
					var u = base + '/mgr_listservassjson.html?status=Y';
					oServAssTable.ajax.url(u).load();
				});
				//添加按钮
	...
	rebindEvent:function(){
		...
		$("[id^='editsrvassign']").unbind().click(function(){
			...
			console.log(id);
			$.ajax({
				url: base + '/mgr_getservassjson.html?id=' + id,
				type: 'GET',
				dataType: 'json',
				success: function (resp) {
					console.log(resp);
			...
			buttons: [
						{
							...
							click: function() {
								...
								console.log(reqData);
								$.ajax({
									url: base + '/mgr_addservassjson.html',
									data: reqData,
									type: 'POST',
									dataType: 'json',
									success: function (resp) {
										var u = base + '/mgr_listservassjson.html?status=Y&a=a';
										oServAssTable.ajax.url(u).load();
										alert(resp.message);
									}
								});
			...
			buttons: [
						{
							...
							click: function() {
								...
								console.log(reqData);
								$.ajax({
									url: base + '/mgr_modifyservassjson.html',
									data: reqData,
									type: 'POST',
									dataType: 'json',
									success: function (resp) {
			...
			buttons: [
						{
							...
							click: function() {
								...
								console.log(reqData);
								$.ajax({
									url: base + '/mgr_editservassjson.html',
									data: reqData,
									type: 'POST',
									dataType: 'json',
									success: function (resp) {
